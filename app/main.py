...
async def compare_timesheets(
    ...
    sap_long = sap_long[['Email', 'Full Name', 'Date', 'Hours']].dropna(subset=[
                                                                        'Date', 'Hours'])
    sap_long['Date'] = pd.to_datetime(sap_long['Date'])
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter', datetime_format='dd-mmm-yyyy') as writer:
        ...
        header_format = workbook.add_format(
            {'bold': True, 'text_wrap': True, 'valign': 'center', 'fg_color': '#DDEBF7', 'border': 1})
        ...
        summary_ws.set_column('C:D', 30)
        summary_ws.set_column('E:G', 18)
